...
class PenyaluranBantuanPpksResource extends Resource
{
                ...
                Group::make()->schema([
                    Section::make()->schema([
                        Select::make('bantuan_ppks_id')
                            ...
                            ->columnSpanFull()
                            ->helperText(str('**Data Diambil dari Program PPKS** Pastikan mengisi terlebih dahulu data Program PPKS.')
                                ->inlineMarkdown()->toHtmlString())
                            ->optionsLimit(20),
                        TextInput::make('no_kk')
                            ->label('Nomor Kartu Keluarga (KK) KPM')
                            ->disabled()
                            ->dehydrated(),
                        TextInput::make('nik')
                            ->label('Nomor Induk Kependudukan (NIK) KPM')
                            ->disabled()
                            ->dehydrated(),
                        Geocomplete::make('lokasi')
                            ->label('Lokasi Alamat KPM')
                            ->placeholder('Ketikkan alamat atau pilih di peta')
                            ->required()
                            ->countries(['id'])
                            ->updateLatLng()
                            ->geocodeOnLoad()
    ...
    public static function getEloquentQuery(): Builder
    {
        ...
        return parent::getEloquentQuery()
            ->whereHas('bantuan_ppks', fn(Builder $query) => $query->where('kelurahan', auth()->user()->instansi_code))
            ->withoutGlobalScopes([
                SoftDeletingScope::class,
            ]);
    }
    ...
    public static function getPages(): array
    {
        return [
            ...
            'view' => Pages\ViewPenyaluranBantuanPpks::route('/{record}'),
